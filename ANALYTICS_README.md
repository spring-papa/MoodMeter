...
- [ ] 설정 탭 클릭 시 `settings_tab_click` + `screen_view(view_type=settings)` 이벤트
- [ ] 설정 저장/초기화 시 `settings_action_click` 이벤트
...
### 2-1. 설정 액션 분석
**질문**: "설정에서 어떤 액션(저장/초기화)이 자주 발생하는가?"

**GA4 탐색 방법**:
1. Analytics → Explore → Blank
2. Dimensions: `setting_key`, `action_type`, `result`
3. Metrics: `Event count` (`settings_action_click` 필터)
...
### 설정 이벤트 스키마 (확장형)
- 이벤트명: `settings_action_click`
- 권장 파라미터:
  - `setting_key` (예: `display_name`, `font_size`, `notification_enabled`)
  - `action_type` (`save`, `reset`, `toggle`, `select`, `open`)
  - `input_method` (`button`, `switch`, `select`, `text_input`)
  - `result` (`success`, `fail`, `validation_error`)
  - `value_type` (`string`, `boolean`, `number`, `enum`)
  - `value_length` (문자열 길이)
  - `is_default_name` (현재 `display_name`에 대해 사용)
...
│   └── app.js                # 메인 앱 (설정/이름치환/localStorage/Analytics 포함)
